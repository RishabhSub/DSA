...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // cout<<"head_: "<<head_<<" head_->data: "<<head_->data<<endl;
  // cout<<"tail_: "<<tail_<<" tail_->data: "<<tail_->data<<endl;
  
  // curr = head_;
  // int index = 0;
  // while (index <= size_) {
    // cout<<"curr->data: "<<curr->data<<" curr->prev: "<<curr->prev<<" curr: "<<curr<<" curr->next: "<<curr->next<<endl;
    // curr = curr->next;
    // index++;
  // }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node* leftHead = left.getHeadPtr();
  Node* rightHead = right.getHeadPtr();
  Node* mergedHead = merged.getHeadPtr();
  
  Node* currLeft = leftHead;
  Node* currRight = rightHead;
  Node* currMerged = mergedHead;
  
  while (currLeft != nullptr && currRight != nullptr) {
    if (currLeft->data < currRight->data) {
      // Insert currLeft into back of merged
      // Increment currLeft
      merged.pushBack(currLeft->data);
      cout<<"currLeft->data less than currRight->data"<<endl;
      currLeft = currLeft->next;
    }
    else {
      // Insert currRight at back of merged
      merged.pushBack(currRight->data);
      cout<<"currRight->data less than currLeft->data"<<endl;
  ...
  while (currLeft == nullptr || currRight == nullptr) {
    if (currLeft != nullptr && currRight == nullptr) {
      merged.pushBack(currLeft->data);
      cout<<"only currLeft is remaining"<<endl;
      currLeft = currLeft->next;
    }  
    else if (currRight != nullptr && currLeft == nullptr) {
      merged.pushBack(currRight->data);
      cout<<"only currRight is remaining"<<endl;
      currRight = currRight->next;
    }
